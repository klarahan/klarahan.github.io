...
chosun <- readxl::read_xls("data/all_issues.xls", sheet = "Chosun_?Üµ?ùº") %>%
...
hani <- bind_rows(readxl::read_xls("data/all_issues.xls", sheet = "Hani_?Üµ?ùº")) %>%
...
# all_news %>%
#     filter(stringr::str_detect(Title, "Í∞ïÏòÅ?õàÏ¥ùÎ¶¨ Í∏∞Ï°∞?ó∞?Ñ§"))
...
all_news <- bind_rows(chosun, hani) %>%
  ...
  filter(!stringr::str_detect(Body, "Í≥µÌï≠?å®?Öò")) 
...
```{r}
all_news %>%
  filter(stringr::str_detect(Body, "?ïµ")) %>% 
  dplyr::mutate(article =  row_number()) %>%
  tidytext::unnest_tokens(word, Body) %>% 
  dplyr::anti_join(stopwords) %>% 
  inner_join(senti) %>%
